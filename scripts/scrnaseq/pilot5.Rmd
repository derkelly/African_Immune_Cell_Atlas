...
for (i in 1:3){
  ...
  gex_raw <- Read10X(data.dir = paste0("count/tk_p5_L", i, "/outs/filtered_feature_bc_matrix"))
  # rename columns to exclude the trailing number
  colnames(gex_raw) <- get_stem(colnames(gex_raw))
  
  # read in HTO data
  hto_raw <- Read10X(data.dir = paste0("citeseqcount/L", i, "_HTO/umi_count"), gene.column=1)
  ...
  if (i < 3){
    ...
    # read in ADT data
    adt_raw <- Read10X(data.dir = paste0("citeseqcount/L", i, "_ADT/umi_count"), gene.column=1)
    adt_raw <- adt_raw[!(rownames(adt_raw)=="unmapped"),] # remove unmapped counts
    adt_raw.rownames <- sapply(rownames(adt_raw),
                               function(x) paste(head(unlist(strsplit(x, "-")), -1), collapse = "_"),
                               USE.NAMES = F)# rename rows
    rownames(adt_raw) <- adt_raw.rownames
    adt <- CreateAssayObject(adt_raw)
    gex[["ADT"]] <- adt
  ...
  samp_stim = data.frame(HTO_maxID=gex$HTO_maxID) %>% 
    separate(HTO_maxID, into=c("sample","tech","stim","bc"), sep="-")
  
  gex$sample = samp_stim$sample
  gex$tech = samp_stim$tech
